...
void loop() {
  ...
  //Create some input instructions to operate the rover (Command)
    ...
    Serial.print("Instruction received");Serial.println(instr);
     Instr_Decode(instr);
    }
    ...
    if((millis()-instructionStartTime > 70000 && !instructionCompleted) || emergencyStop || S){
      Serial.println("Failed to execute instruction within 20 seconds, or emergency or normal stop");
      emergencyStop=0;
      if(F || B){
      ...
      if(L || R){
        Send_Instruction_Completed((-total_x/arc)*90,0,(L ? 'L':'R'));
        }
   ...
   if(F){
      ...
      cp = pi_d(err_y);
      Speed_Control(abs(cp)*0.01+0.38); //Using PI controller result to control speed
  ...
  if(B){
   ...
   if(L){
      ...
      if(err_x >= -0.2 && err_x <= 0.2){
         ...
         if(millis()-instructionCompleteTime > 10000 && !firstTime && !instructionCompleted){
           instructionCompleted=1;
           Send_Instruction_Completed((-total_x/arc)*90,1,'L');
         }
         Stop();
   ...
   if(R){
      ...
      if(err_x >= -0.2 && err_x <= 0.2){
         ...
         if(millis()-instructionCompleteTime > 10000 && !firstTime && !instructionCompleted){
           instructionCompleted=1;
           Send_Instruction_Completed((-total_x/arc)*90,1,'R'); //how far the rover moves
         }
         Stop();
...
void Instr_Decode(String instruction){
  ...
  if(instruction[0] == 'P'){ //pause
    ...
    delay(100);
    Serial1.write('p');
    Serial1.print(String(total_x)); // total x

    while(!Serial1.available() || Serial1.peek()!='N'){ //unpause
      delay(1); //50ms
    }
    Serial1.read(); //get rid of the X
    Serial1.print('y');
    Serial1.print(String(total_y)); // total y
...
float Angle_Conversion(float target_angle){
  return (target_angle/90)*arc;  //Converting from input angle to target dist. along x
  }
